...
function buildRecipie(){
  ...
  items.forEach(element => {
    if(element.querySelector('.check').checked){
      ...
      let pid = getPidFromCart(element.id);

      let item = {'pid': pid, 'quantidade': quantidade, 'preco': tPrice};
  ...
  let data = {'uid': user['uid'], 'preco': total, estado: 'Aberto', produtos: array, 'cartIds': cartId, 'request_type':'write'};
